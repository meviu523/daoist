(function(root){
  ...
  const actionCard=(name,description,action,attrs='',symbol='spark',off=false)=>{
    if(state&&(action.startsWith('panel-')?!G.panelUnlocked(state,action.slice(6)):!!G.featureBlock(state,action)))return '';
    return `<button class="action-card" data-act="${action}" ${attrs}${disabled(off)}>${icon(symbol)}<div><strong>${name}</strong><small>${description}</small></div><span class="arrow">›</span></button>`;
  };
  ...
  function onPlace(id){if(!state)return;if(state.gameOver){openPanel('gameover');return;}if(state.eventResult){openPanel('result');return;}if(state.pending){openPanel('event');return;}if(state.ending){openPanel('ending');return;}map.select(id);openPanel('place',{id});}
  function openPanel(type,data={}){if(state?.eventResult&&type!=='gameover')type='result';if(state&&!G.panelUnlocked(state,type)){const f=G.FEATURE_UNLOCKS.find(f=>f.id===G.FEATURE_PANELS[type]);showError(`${f.name}尚未开启：${f.hint}。`);return;}clock.pause('panel');if(state)persist();dialogType=type;dialogData=data;clearError();renderPanel();dialog.classList.toggle('centered',!state||type==='new'||type==='gameover');if(!dialog.open)dialog.showModal();renderLive();if(type==='new')setTimeout(()=>$('#player-name-input')?.focus(),40);if(type==='result')queueMicrotask(()=>{if(dialogType==='result')dialog.querySelector('[data-act="ackResult"]')?.focus();});}
  function closePanel(){if(state?.gameOver||state?.eventResult||state?.pending||state?.ending)return;dialog.close();dialogType=null;dialogData={};map?.select(null);clearError();syncPause();renderLive();}
  function panelTemplate(title,eyebrow,body,foot=''){
    const closable=!(state?.gameOver||state?.eventResult||state?.pending||state?.ending);
  ...
  function routeButton(target,label='前往这里'){
    const locationBlock=G.locationBlock(state,'travel',{target});
    if(locationBlock)return `<p class="copy location-locked">${esc(locationBlock)}</p>`;
  ...
  function renderPanel(){
    ...
    switch(dialogType){
      ...
      case 'place':{
        const p=G.place(dialogData.id);if(!p){closePanel();return;}title=p.name;label='City destination';const known=G.placeUnlocked(s,p.id);
        body=`<div class="row">${tag(!known?'送达后解锁':s.position===p.id?'当前位置':p.permanent?'城市地点':'配送目的地',s.position===p.id?'mint':'')}<span class="muted" style="font-size:11px">${s.transport==='bike'?'电动车出行':'步行推车'}</span></div>${known&&p.desc?`<p class="copy" style="margin-top:12px">${esc(p.desc)}</p>`:''}`;
        const orders=s.orders.filter(o=>o.target===p.id);
        if(s.activeOrder?.target===p.id)body+=`<p class="alert soft">已接订单：${esc(s.activeOrder.title)}。接单后不会再消失，也没有送达时限。</p>${btn('继续当前配送',`data-act="resumeDelivery"${disabled(!!s.activity)}`,'wide')}`;
        for(const order of orders){const plan=G.travelPlan(s,p.id);let block=s.activeOrder?'已有未完成的配送订单。':G.travelBlock(s,plan);if(!block&&order.expiresAt<=s.minutes)block='这张订单已经从地图消失。';if(!block&&order.condition==='mystic'&&s.player.mana<8)block='灵异订单需要至少 8 点灵力。';if(!block&&order.condition==='careful'&&s.player.stamina-plan.stamina<10)block='易损餐品需要抵达时至少保留 10 点体力。';
          body+=`<article class="order-box"><div class="row"><h3>${esc(order.title)}</h3>${tag(`${G.durationText(G.orderOfferRemaining(s,order))}后消失`,'gold')}</div><p class="copy" style="margin-top:10px">${esc(order.desc)}</p><div class="order-reward"><span>¥${order.reward}<small> 现金</small></span><span>${order.coins}<small> 外卖币</small></span></div>${travelLine(p.id)}<p class="copy">消失时间只限制接单；接下后可按自己的路线完成配送。</p>${!known?'<p class="copy location-locked">首次送达并完成事件选择后，永久解锁此地点。</p>':''}${order.condition==='mystic'?'<p class="copy">交付需要灵力 −8。</p>':''}${block?`<div class="alert" style="margin:12px 0">${esc(block)}</div>`:''}${btn('接单并配送 →',`data-act="deliver" data-id="${esc(order.id)}"${disabled(!!block)}`,'primary wide')}</article>`;
        }
        if(p.permanent&&known){body+='<div class="section-title"><h3>在这里</h3></div>';if(s.position!==p.id)body+=routeButton(p.id);else{
        ...
        }else if(!orders.length&&s.activeOrder?.target!==p.id)body+='<p class="copy" style="margin-top:16px">这里暂时没有订单。地点需通过外卖送达解锁，请在补单后留意地图。</p>';
      ...
      case 'cultivation':{
        title='今夜，向内而行';label='Cultivation';
        const need=G.realmNeed(s),nextRealm=G.nextRealmLabel(s),atMax=G.realmAtMax(s),advanced=G.featureUnlocked(s,'practice'),foundationTier=G.highestPillTier(s,'foundation'),foundationCount=G.pillCount(s,'foundation');
        if(!foundationCount)usePill=false;
        const chance=G.breakChance(s,usePill?foundationTier:0);
        body=`<div class="row"><h3>${G.realmLabel(s)}${nextRealm?` <span class="muted">→</span> ${nextRealm}`:''}</h3>${tag(G.isNight(s)?'夜间灵气 +45%':'白昼 · 灵气平缓',G.isNight(s)?'gold':'')}</div><div class="wide-progress"><i style="width:${need?Math.min(100,s.player.qi/need*100):100}%"></i></div><div class="progress-label"><span>修为</span><span>${Math.floor(s.player.qi)}${need?` / ${need}`:` · ${G.realmLabel(s)}圆满`}</span></div><div class="section-title"><h3>功课</h3>${s.position==='temple'?tag('灵地 +25%','mint'):tag(G.locationName(s))}</div><div class="stack">${actionCard('吐纳','45 分钟 · 体力 −12 · 恢复灵力','cultivate','data-kind="breath"','lotus',s.player.stamina<12)}${advanced?actionCard('静坐入定','90 分钟 · 体力 −24 · 更多修为','cultivate','data-kind="meditate"','moon',s.player.stamina<24)+actionCard('淬体练步','30 分钟 · 体力 −20 · 积累根骨与身法','cultivate','data-kind="body"','sword',s.player.stamina<20):''}</div>`;
        if(!advanced)body+='<p class="copy" style="margin-top:16px">先完成一次吐纳，再开启进阶修行与街巷探索。中途停止不算完成。</p>';
        else{
          body+=`<div class="section-title"><h3>叩问下一境</h3>${!atMax?tag(`成功率 ${Math.floor(chance)}%`,'gold'):tag(`${G.realmLabel(s)} · 道心已明`)}</div>${!atMax?`<label class="checkbox"><input type="checkbox" id="break-pill"${usePill?' checked':''}${disabled(!foundationCount)}>使用最高阶破境丹（共 ${foundationCount} 枚${foundationTier?` · 最高 ${foundationTier} 阶 · 成功率 +${G.breakthroughPillBonus(foundationTier)}%`:''}）</label><p class="copy" style="margin:11px 0">下一境：${nextRealm} · 需要 ${need} 修为。${(s.inventory.charm||0)>0?'背包中的护身符会在失败时自动保护你。':'失败会损失本次需求的 25% 修为与 18 气血。'}突破耗时 60 分钟、消耗 20 体力。</p>${btn('尝试突破',`data-act="breakthrough"${disabled(s.player.qi<need||s.player.stamina<20)}`,'primary wide')}`:G.featureUnlocked(s,'endings')?btn('看看自己的归途','data-ui="endings"','primary wide'):''}`;
      ...
      case 'alchemy':{
        ...
        const alchemyRank=G.ALCHEMY_RANKS[G.alchemyRank(s)],cauldron=G.cauldron(s),atMarket=s.position==='market'&&G.placeUnlocked(s,'market'),pillCap=(s.alchemy?.cauldron||0)?Math.min(cauldron.tier,G.alchemistTier(s)):0,ownedCauldrons=Array.isArray(s.alchemy?.cauldrons)?s.alchemy.cauldrons:[],ownedRecipes=G.ALCHEMY_RECIPES.filter(r=>G.alchemyHasFormula(s,r)),formulaCandidates=G.formulaMarketCandidates(s),formulaCost=G.formulaScrollCost(s);
        const cauldronGroups=G.CAULDRONS.slice(1).filter(d=>s.player.realm>=d.realm||ownedCauldrons.includes(d.level)).reduce((out,d)=>{(out[d.tier]||(out[d.tier]=[])).push(d);return out;},{});
        const cauldronCatalog=atMarket?Object.entries(cauldronGroups).map(([tier,list])=>'<details class="card"'+(Number(tier)===(cauldron.tier||1)?' open':'')+'><summary><strong>'+tier+'阶药鼎</strong> · '+list.length+' 种</summary><div class="stack" style="margin-top:12px">'+list.map(d=>{const owned=ownedCauldrons.includes(d.level),active=s.alchemy.cauldron===d.level,locked=s.player.realm<d.realm;return '<div class="card"><div class="row"><div><h3>'+esc(d.name)+'</h3><p class="copy">成丹修正 +'+Math.round(d.success*100)+'% · 额外产出 '+Math.round(d.extra*100)+'% · 成色修正 '+Math.round(d.quality*100)+'%</p></div>'+tag(d.tier+'阶',active?'gold':'')+'</div>'+btn(active?'当前使用':owned?'换用':locked?G.REALMS[d.realm].name+'后可用':'购买 · ¥'+d.cost,'data-act="cauldron" data-id="'+d.level+'"'+disabled(active||locked||(!owned&&s.player.money<d.cost)),'small wide')+'</div>';}).join('')+'</div></details>').join(''):'<p class="copy">24 种药鼎分为九阶，同阶器型有不同成丹、额外产出与成色侧重。已拥有 '+ownedCauldrons.length+' / 24。</p>'+routeButton('market','前往长乐集选药鼎');
        const recipeGroups=[...new Set(ownedRecipes.map(r=>r.route))].map((route,groupIndex)=>{
          const cards=ownedRecipes.filter(r=>r.route===route).map(recipe=>{
            const qualified=G.alchemyQualified(s,recipe),hasDing=(s.alchemy?.cauldron||0)>0,cauldronReady=G.alchemyCauldronReady(s,recipe),materialsReady=Object.entries(G.alchemyMaterials(recipe)).every(([id,count])=>(s.inventory[id]||0)>=count),ready=hasDing&&qualified&&cauldronReady&&materialsReady&&s.player.mana>=recipe.mana,chanceText=hasDing?Math.floor(G.alchemyChance(s,recipe)*100)+'%':'需药鼎';
            let actionLabel='开炉炼制';if(!qualified)actionLabel='暂不可参悟';else if(!hasDing)actionLabel='先买药鼎';else if(!cauldronReady)actionLabel='药鼎阶位不足';else if(!materialsReady)actionLabel='药材不足';else if(s.player.mana<recipe.mana)actionLabel='灵力不足';
            return '<div class="card"><div class="row"><div><h3>'+esc(recipe.name)+'</h3><p class="copy">获取途径：'+esc(G.formulaSourceLabel(recipe))+'</p><p class="copy">产物：'+esc(G.pillType(recipe.product).name)+' · 丹方复杂度 '+recipe.complexity+'</p></div>'+tag(chanceText,ready?'gold':'')+'</div><p class="copy">'+esc(G.alchemyMaterialText(recipe))+' · 灵力 '+recipe.mana+' · 火候 '+recipe.duration+' 分钟 · 至少 '+recipe.minCauldronTier+' 阶药鼎</p><p class="copy">'+(qualified?'已参悟':'需要熟练度 '+recipe.need+(recipe.realm?' · '+G.REALMS[recipe.realm].name:''))+' · 成品阶数由药鼎与炼药师共同决定。</p>'+btn(actionLabel,'data-act="alchemy" data-recipe="'+recipe.id+'"'+disabled(!ready),'small wide')+'</div>';
          }).join('');
          return '<details class="card"'+(groupIndex===0?' open':'')+'><summary><strong>'+esc(route)+'</strong> · '+ownedRecipes.filter(r=>r.route===route).length+' 张已获得丹方</summary><div class="stack" style="margin-top:12px">'+cards+'</div></details>';
        }).join('');
        const formulaAcquire=atMarket
          ? '<div class="card"><div class="row"><div><h3>商店丹方残卷</h3><p class="copy">仅售商店丹方，不含剧情与羁绊专属传承。购买后才会辨认具体内容并永久收录。</p></div>'+tag('已收录 '+ownedRecipes.length+' 张','mint')+'</div><p class="copy">当前境界还有 '+formulaCandidates.length+' 卷商店丹方未收录。</p>'+btn(formulaCandidates.length?'购入一卷未收录丹方 · ¥'+formulaCost:'当前境界商店丹方已收齐','data-act="formula"'+disabled(!formulaCandidates.length||s.player.money<formulaCost),'small wide')+'</div>'
          : '<div class="card"><h3>丹方收录</h3><p class="copy">丹方分剧情解锁、羁绊传授和商店购买。只有已获得丹方才显示具体内容；商店不会出售专属传承。</p>'+routeButton('market','前往长乐集寻丹方')+'</div>';
        const materialGroups=G.ALCHEMY_MATERIALS.filter(m=>s.player.realm>=m.realm).reduce((out,m)=>{(out[m.tier]||(out[m.tier]=[])).push(m);return out;},{});
        const materialShop=atMarket?Object.entries(materialGroups).map(([tier,list])=>'<details class="card"><summary><strong>'+tier+'阶药材</strong> · '+list.length+' 种</summary><div class="button-row" style="margin-top:12px">'+list.map(m=>btn(esc(m.name)+' · ¥'+m.price,'data-act="material" data-id="'+m.id+'"'+disabled(s.player.realm<m.realm||s.player.money<m.price),'small')).join('')+'</div></details>').join(''):'<p class="copy">64 种药材在长乐集按境界开放购买；月渡公园与听雨观探索也可能采得当前层级的药材。</p>'+routeButton('market','前往长乐集选药材');
        body=`<div class="section-title"><h3>炼药师</h3>${tag(alchemyRank.name+' · '+alchemyRank.tier+'阶 · 熟练度 '+(s.alchemy?.xp||0),'mint')}</div><div class="card"><div class="row"><div><h3>${cauldron.name}</h3><p class="copy">${cauldron.desc}</p></div>${(s.alchemy?.cauldron||0)>0?tag(cauldron.tier+'阶 · 器型 '+s.alchemy.cauldron+'/24','gold'):tag('尚未购鼎')}</div><p class="copy">已开炉 ${s.alchemy?.brews||0} 次 · 成丹 ${s.alchemy?.successes||0} 次 · 已拥有 ${ownedCauldrons.length}/24 口鼎 · ${pillCap?`当前成品阶数上限 ${pillCap} 阶`:'尚无成品阶数上限'}</p></div><div class="section-title"><h3>药鼎</h3><span class="muted" style="font-size:11px">24 种 · 九阶</span></div><div class="stack">${cauldronCatalog}</div>
          <div class="section-title"><h3>药材</h3><span class="muted" style="font-size:11px">64 种 · 九阶分布</span></div><div class="stack">${materialShop}</div>
          <div class="section-title"><h3>丹方</h3><span class="muted" style="font-size:11px">只显示已获得 · 已收录 ${ownedRecipes.length} 张</span></div><div class="stack">${formulaAcquire}</div><p class="copy" style="margin:12px 0">剧情、羁绊、商店各有专属丹方；获得后才显示具体内容。丹方决定配伍和火候，丹药阶数仍由药鼎与炼药师共同决定。</p><div class="stack">${recipeGroups||'<div class="empty">尚未获得任何丹方。</div>'}</div>`;break;
      }
      case 'system':{
        title='万家灯火系统';label='The lantern system';if(systemTab==='endings'&&!G.featureUnlocked(s,'endings'))systemTab='quests';const story=G.storyStage?G.storyStage(s):null;body=`<div class="row" style="margin-bottom:20px"><span class="gold-text" style="font-size:22px">${s.player.coins} <small style="font-size:11px">外卖币</small></span>${story?tag(story.title,'mint'):''}${btn(s.daily.signedDay===G.day(s)?'今日已签到':'每日签到',`data-act="sign"${disabled(s.daily.signedDay===G.day(s))}`,'small soft')}</div><div class="tabs" role="tablist" aria-label="系统分类">${[['shop','兑换'],['quests','委托'],['endings','归途']].filter(([id])=>id!=='endings'||G.featureUnlocked(s,'endings')).map(([id,n])=>`<button role="tab" aria-selected="${id===systemTab}" class="${id===systemTab?'active':''}" data-ui="system-tab" data-id="${id}">${n}</button>`).join('')}</div>`;
        if(systemTab==='shop'){
          body+=`<div class="shop-grid">${G.ITEMS.filter(item=>item.shop!==false&&G.itemUnlocked(s,item)).map(item=>{const owned=item.unique&&(s.learned.includes(item.id)||s.equipment.includes(item.id)),unlocked=G.itemUnlocked?G.itemUnlocked(s,item):true,unlockName=G.REALMS[item.unlockRealm||0]?.name||'凡人';return `<article class="shop-item"><span class="item-glyph">${item.glyph}</span><h3>${item.name}</h3><p>${item.desc}</p>${!unlocked?`<p class="muted" style="font-size:11px;margin:0 0 10px">${unlockName}后开放</p>`:''}${btn(owned?'已拥有':unlocked?`${item.cost} 币 · 兑换`:`${unlockName}后解锁`,`data-act="buy" data-id="${item.id}"${disabled(owned||!unlocked||s.player.coins<item.cost)}`,'small')}</article>`;}).join('')}</div>`;foot='远程即时兑换 · 随境界开放 · 不消耗游戏时间';
        }else if(systemTab==='quests'){
          body+=`<div class="stack"><article class="quest-card"><div class="row"><h3>每日 · 三餐有着落</h3>${tag(`${Math.min(3,s.daily.delivered)} / 3`,'mint')}</div><small>当天完成 3 单配送</small><div class="quest-reward">外卖币 +5 · 现金 +30 · 修为 +10</div>${btn(s.daily.claimed?'已领取':'领取奖励',`data-act="claim" data-id="daily"${disabled(s.daily.claimed||s.daily.delivered<3)}`,'small wide')}</article>${G.visibleQuests(s).map(q=>{const done=s.claimed.includes(q.id);return `<article class="quest-card ${done?'complete':''}"><div class="row"><h3>${q.title}</h3>${tag(done?'完成':`${Math.min(s.stats.delivered,q.delivery)}/${q.delivery}`)}</div><small>${q.desc}</small><div class="quest-reward">${G.effectText(q.reward)}</div>${btn(done?'已领取':'领取篇章奖励',`data-act="claim" data-id="${q.id}"${disabled(done||!G.questReady(s,q))}`,'small wide')}</article>`;}).join('')}</div>`;
      ...
      case 'inventory':{
        ...
        body=items.length?`<div class="stack">${items.map(item=>`<article class="card"><div class="row"><h3>${item.name} <span class="badge-number">${s.inventory[item.id]}</span></h3>${item.effect?btn('使用',`data-act="use" data-id="${item.id}"`,'small'):tag(item.type==='pill'?'突破用':'材料')}</div><p class="copy">${item.desc}</p></article>`).join('')}</div>`:'<div class="empty">行囊很轻。系统里可以兑换丹药与材料。</div>';
      ...
      case 'auction':{
        ...
        const at=s.position===G.AUCTION.place&&G.placeUnlocked(s,G.AUCTION.place),today=s.auction.day===G.day(s),hours=G.auctionHours();
      ...
      case 'rest':{
        ...
        const housing=G.visibleResidences(s).map(h=>{const trip=G.travelPlan(s,h.place),tripBlock=G.locationBlock(s,'moveHome',{id:h.id})||G.travelBlock(s,trip),current=h.id===s.residenceId;return `<article class="card"><div class="row"><div><h3>${esc(h.name)} ${current?'<span class="tag mint">当前住处</span>':''}</h3><p class="copy">${esc(h.desc)}</p></div>${tag(`周租 ¥${h.rent}`,current?'gold':'')}</div><p class="copy">${esc(G.residenceRecoveryText(h))}</p>${current?'<p class="copy">房租每 7 天自动结算。</p>':`${travelLine(h.place)}${tripBlock?`<p class="alert">${esc(tripBlock)}</p>`:''}${btn(`搬入${esc(h.name)}`,`data-act="moveHome" data-id="${h.id}"${disabled(!!tripBlock)}`,'small wide')}`}</article>`;}).join('');
      ...
      case 'result':{
        const r=s.eventResult;if(!r){closePanel();return;}title='事件结果';
        const receipt=changes=>changes.length?`<div class="result-changes">${changes.map(x=>`<span class="result-change ${x.delta<0?'cost':'gain'}">${esc(G.eventResultLabel(x.key))} <strong>${x.delta>0?'+':'−'}${esc(Number(Math.abs(x.delta).toFixed(2)))}</strong></span>`).join('')}</div>`:'<p class="copy">没有实际数值变化（可能已达上限）。</p>';
        body=`<article class="event-result" aria-label="事件结算结果"><div class="row"><h3>${esc(r.title)}</h3>${tag(r.source==='ai'?'AI 剧情':'经典剧情','gold')}</div><p class="result-selection">你的选择：${esc(r.choice)}</p><div class="event-text result-text">${esc(r.text)}</div><h3>本次选择</h3>${receipt(r.changes)}${r.duration?`<p class="copy">处理耗时：${r.duration} 个游戏分钟（已完成）</p>`:''}${r.delivery?`<section class="result-delivery"><h3>配送已结算</h3><p class="copy">${esc(r.delivery.title)} · ${esc(G.place(r.delivery.target).name)}</p>${receipt(r.delivery.changes)}</section>`:''}${btn(s.pending?'继续剧情':'继续',`data-act="ackResult" data-id="${esc(r.id)}"`,'primary wide')}</article>`;
        foot='以上为已发生的实际变化，结果已自动保存。确认不会再次扣费或发奖；历史记录仍在右下角日志。';break;
      }
      case 'event':{
    ...
    if(s?.activity)for(const button of dialog.querySelectorAll('[data-act]')){
      if(!['ackResult','buy','sign','claim','stop','panel-cultivation','panel-alchemy','panel-vehicle','panel-auction'].includes(button.dataset.act))button.disabled=true;
    }
  }
  function loadGame(id,autoStart=false){const found=store.saves.find(s=>s.id===id);if(!found)return showError('没有找到这个存档。');abortAI();clock.pause('manual');clock.speed=1;state=G.clone(found);systemTab='shop';usePill=false;conflict=false;dirtySinceSave=false;lastSavedAt=Date.now();saveOk=!store.unavailable;dialog.close();dialogType=null;clearError();renderGame();requestAnimationFrame(()=>map.center());if(state.gameOver)openPanel('gameover');else if(state.eventResult)openPanel('result');else if(state.pending){openPanel('event');maybeGenerateAI();}else if(state.ending)openPanel('ending');if(autoStart)clock.release('manual');syncPause();renderLive();}
  function act(action,payload={}){
    ...
    else if(state.eventResult)openPanel('result');else if(state.pending){openPanel('event');maybeGenerateAI();}
    else if(state.ending)openPanel('ending');
    else if(oldPendingKind==='auction'&&!state.activity)openPanel('auction');
    else if((state.activity&&!['buy','sign','claim'].includes(action))||['ackResult','deliver','travel','visit','choose','sleep','moveHome','continue','stop','cancelDelivery','resumeDelivery'].includes(action)){closePanel();}
    else if(dialogType){renderPanel();}
    syncPause();renderLive();
  }
  async function maybeGenerateAI(){
    const s=state,p=s?.pending;if(!s||s.eventResult||s.mode!=='ai'||!p||p.aiStatus!=='unrequested'||aiFlight?.id===p.id)return;
  ...
  document.addEventListener('click',e=>{
    ...
    if(button.dataset.panel){if(state?.gameOver)openPanel('gameover');else if(state?.eventResult)openPanel('result');else if(state?.pending)openPanel('event');else if(state?.ending)openPanel('ending');else openPanel(button.dataset.panel);return;}
    ...
    switch(d.ui){
      ...
      case 'system-tab':if(!state||!G.featureUnlocked(state,'system')||!['shop','quests','endings'].includes(d.id)||(d.id==='endings'&&!G.featureUnlocked(state,'endings')))break;systemTab=d.id;renderPanel();break;
      case 'quests':systemTab='quests';if(state?.eventResult)openPanel('result');else if(state?.pending)openPanel('event');else openPanel('system');break;
      case 'endings':if(!state||!G.featureUnlocked(state,'endings'))break;systemTab='endings';if(state?.eventResult)openPanel('result');else if(state?.pending)openPanel('event');else openPanel('system');break;
      case 'confirm-ending':if(!state||!G.featureUnlocked(state,'endings'))break;if(confirm('写下这个结局？之后仍然可以继续游历。'))act('finale',{id:d.id});break;
    }
  });
  ...
  dialog.addEventListener('cancel',e=>{if(state?.gameOver||state?.eventResult||state?.pending||state?.ending)e.preventDefault();else{dialogType=null;map?.select(null);}});
  ...
  function syncPause(){
    for(const [reason,blocked] of [['menu',!state],['panel',dialog.open],['event',!!(state?.eventResult||state?.pending||state?.ending||state?.gameOver)],['conflict',conflict]]){
  ...
  function renderLive(){
    ...
    const detail=a?.phase==='travel'?`剩余 ${Math.ceil(a.route.meters-a.travelled)} 米 · 预计 ${G.durationText(G.activityRemaining(s))}`:a?`剩余 ${G.durationText(G.activityRemaining(s))}`:s.activeOrder?`${s.activeOrder.title} · 已接单，无送达时限`:`${clock.paused?'点击顶部继续':'时间正在流逝'} · 距房租 ${Math.max(0,Math.ceil(until/60))} 小时 / ¥${home.rent}`;
