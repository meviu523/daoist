  ...
  function renderPanel(){
    ...
    switch(dialogType){
      ...
      case 'cultivation':{
        ...
        const need=G.realmNeed(s),nextRealm=G.nextRealmLabel(s),atMax=G.realmAtMax(s),foundationTier=G.highestPillTier(s,'foundation'),foundationCount=G.pillCount(s,'foundation'),chance=G.breakChance(s,usePill?foundationTier:0),alchemyRank=G.ALCHEMY_RANKS[G.alchemyRank(s)],cauldron=G.cauldron(s),nextCauldron=G.CAULDRONS[(s.alchemy?.cauldron||0)+1],atMarket=s.position==='market',pillCap=Math.min(cauldron.tier||1,G.alchemistTier(s));
        if(!foundationCount)usePill=false;
        const cauldronControl=nextCauldron?(atMarket?btn(((s.alchemy?.cauldron||0)?'换用':'购买')+nextCauldron.name+' · ¥'+nextCauldron.cost,'data-act="cauldron"'+disabled(s.player.realm<nextCauldron.realm||s.player.money<nextCauldron.cost),'small wide'):'<p class="copy">下一口鼎：'+nextCauldron.name+' · '+nextCauldron.tier+'阶 · ¥'+nextCauldron.cost+(nextCauldron.realm?' · '+G.REALMS[nextCauldron.realm].name+'可用':'')+'</p>'+routeButton('market','前往长乐集买鼎')):tag('24 口药鼎已全部收集','gold');
        ...
        body=`<div class="row"><h3>${G.realmLabel(s)}${nextRealm?` <span class="muted">→</span> ${nextRealm}`:''}</h3>${tag(G.isNight(s)?'夜间灵气 +45%':'白昼 · 灵气平缓',G.isNight(s)?'gold':'')}</div><div class="wide-progress"><i style="width:${need?Math.min(100,s.player.qi/need*100):100}%"></i></div><div class="progress-label"><span>修为</span><span>${Math.floor(s.player.qi)}${need?` / ${need}`:' · 化神九重圆满'}</span></div><div class="section-title"><h3>功课</h3>${s.position==='temple'?tag('灵地 +25%','mint'):tag(G.locationName(s))}</div><div class="stack">${actionCard('吐纳','45 分钟 · 体力 −12 · 恢复灵力','cultivate','data-kind="breath"','lotus',s.player.stamina<12)}${actionCard('静坐入定','90 分钟 · 体力 −24 · 更多修为','cultivate','data-kind="meditate"','moon',s.player.stamina<24)}${actionCard('淬体练步','30 分钟 · 体力 −20 · 积累根骨与身法','cultivate','data-kind="body"','sword',s.player.stamina<20)}</div>
          ...
          <div class="section-title"><h3>炼药师</h3>${tag(alchemyRank.name+' · '+alchemyRank.tier+'阶 · 熟练度 '+(s.alchemy?.xp||0),'mint')}</div><div class="card"><div class="row"><div><h3>${cauldron.name}</h3><p class="copy">${cauldron.desc}</p></div>${(s.alchemy?.cauldron||0)>0?tag(cauldron.tier+'阶 · 器型 '+s.alchemy.cauldron+'/24','gold'):tag('尚未购鼎')}</div><p class="copy">已开炉 ${s.alchemy?.brews||0} 次 · 成丹 ${s.alchemy?.successes||0} 次 · 当前成品阶数上限 ${pillCap} 阶</p>${cauldronControl}</div>
