(function(root){
  ...
  function renderPanel(){
    ...
    switch(dialogType){
      ...
      case 'character':{
        ...
        body=`<div class="row">${tag(`${G.REALMS[s.player.realm].name} · ${G.locationName(s)}`,'gold')}${tag(s.mode==='ai'?'AI 剧情':'经典剧情')}</div><div class="stat-grid" style="margin-top:20px">${statsBox('气血',captext(s.player.health,c.health))}${statsBox('体力',captext(s.player.stamina,c.stamina))}${statsBox('灵力',captext(s.player.mana,c.mana))}</div><div class="section-title"><h3>修行资质</h3></div><div class="detail-list">${Object.entries(labels).map(([k,n])=>`<div><span>${n}</span><strong>${s.player[k]}</strong></div>`).join('')}</div><div class="section-title"><h3>沿途留下的印记</h3></div><div class="stat-grid">${statsBox('送达订单',s.stats.delivered,'单')}${statsBox('累计行程',(s.stats.distance/1000).toFixed(1),'km')}${statsBox('配送收入',s.stats.earned,'元')}</div><p class="copy" style="margin:17px 0">已习得：${s.learned.map(id=>G.ITEMS.find(i=>i.id===id).name).join('、')||'暂无心法'}<br>已装备：${s.equipment.map(id=>G.ITEMS.find(i=>i.id===id).name).join('、')||'暂无法器'}</p>${btn(`${icon('download')} 导出当前存档`,'data-ui="export-current"','wide')}`;break;
      }
      case 'cultivation':{
        title='今夜，向内而行';label='Cultivation';const r=G.REALMS[s.player.realm],chance=G.breakChance(s,usePill);
        body=`<div class="row"><h3>${r.name}${r.need?` <span class="muted">→</span> ${G.REALMS[s.player.realm+1].name}`:''}</h3>${tag(G.isNight(s)?'夜间灵气 +45%':'白昼 · 灵气平缓',G.isNight(s)?'gold':'')}</div><div class="wide-progress"><i style="width:${r.need?Math.min(100,s.player.qi/r.need*100):100}%"></i></div><div class="progress-label"><span>修为</span><span>${Math.floor(s.player.qi)}${r.need?` / ${r.need}`:' · 已达化神'}</span></div><div class="section-title"><h3>功课</h3>${s.position==='temple'?tag('灵地 +25%','mint'):tag(G.locationName(s))}</div><div class="stack">${actionCard('吐纳','45 分钟 · 体力 −12 · 恢复灵力','cultivate','data-kind="breath"','lotus',s.player.stamina<12)}${actionCard('静坐入定','90 分钟 · 体力 −24 · 更多修为','cultivate','data-kind="meditate"','moon',s.player.stamina<24)}${actionCard('淬体练步','30 分钟 · 体力 −20 · 积累根骨与身法','cultivate','data-kind="body"','sword',s.player.stamina<20)}</div>
  ...
  async function maybeGenerateAI(){
    ...
    try{
      const context={name:s.name,realm:G.REALMS[s.player.realm].name,day:G.day(s),time:G.clock(s),weather:s.weather,location:G.locationName(s),kind:p.kind,title:p.title,text:p.text,npc:p.npcId?G.NPCS.find(n=>n.id===p.npcId).name:null,stats:{stamina:s.player.stamina,mana:s.player.mana,money:s.player.money,qi:s.player.qi,karma:s.player.karma,rep:s.player.rep},recent:s.logs.slice(-3).map(l=>l.text)};
  ...
  function pageBoundary(save=false){clock.reset();if(save&&!conflict)persist();renderLive();}
  document.addEventListener('visibilitychange',()=>pageBoundary(document.hidden));
  window.addEventListener('pagehide',()=>pageBoundary(true));
  window.addEventListener('pageshow',()=>pageBoundary(false));
