    ...
    switch(dialogType){
      ...
      case 'cultivation':{
        ...
        const formulaAcquire=atMarket
          ? '<div class="card"><div class="row"><div><h3>丹方残卷</h3><p class="copy">未获得丹方不会显示名称、产物、配伍或要求。购入残卷后才会辨认并永久收录。</p></div>'+tag('已收录 '+ownedRecipes.length+' 张','mint')+'</div><p class="copy">当前境界还有 '+formulaCandidates.length+' 卷未收录丹方可寻。</p>'+btn(formulaCandidates.length?'购入一卷未收录丹方 · ¥'+formulaCost:'当前境界已收齐','data-act="formula"'+disabled(!formulaCandidates.length||s.player.money<formulaCost),'small wide')+'</div>'
          : '<div class="card"><h3>丹方收录</h3><p class="copy">只显示已经获得的丹方。长乐集可购入未知残卷，月渡公园与听雨观探索也可能发现丹方。</p>'+routeButton('market','前往长乐集寻丹方')+'</div>';
        ...
        body=`<div class="row"><h3>${G.realmLabel(s)}${nextRealm?` <span class="muted">→</span> ${nextRealm}`:''}</h3>${tag(G.isNight(s)?'夜间灵气 +45%':'白昼 · 灵气平缓',G.isNight(s)?'gold':'')}</div><div class="wide-progress"><i style="width:${need?Math.min(100,s.player.qi/need*100):100}%"></i></div><div class="progress-label"><span>修为</span><span>${Math.floor(s.player.qi)}${need?` / ${need}`:' · 化神九重圆满'}</span></div><div class="section-title"><h3>功课</h3>${s.position==='temple'?tag('灵地 +25%','mint'):tag(G.locationName(s))}</div><div class="stack">${actionCard('吐纳','45 分钟 · 体力 −12 · 恢复灵力','cultivate','data-kind="breath"','lotus',s.player.stamina<12)}${actionCard('静坐入定','90 分钟 · 体力 −24 · 更多修为','cultivate','data-kind="meditate"','moon',s.player.stamina<24)}${actionCard('淬体练步','30 分钟 · 体力 −20 · 积累根骨与身法','cultivate','data-kind="body"','sword',s.player.stamina<20)}</div>
          ...
          <div class="section-title"><h3>丹方</h3><span class="muted" style="font-size:11px">只显示已获得 · 已收录 ${ownedRecipes.length} 张</span></div><div class="stack">${formulaAcquire}</div><p class="copy" style="margin:12px 0">获得丹方后才会显示具体内容；丹方决定药材配伍、火候和炼制难度，丹药阶数仍由药鼎与炼药师共同决定。</p><div class="stack">${recipeGroups||'<div class="empty">尚未获得任何丹方。</div>'}</div>`;break;
      }
      case 'system':{
