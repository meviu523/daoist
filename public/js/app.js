    ...
    switch(dialogType){
      ...
      case 'place':{
        ...
        if(p.permanent){body+='<div class="section-title"><h3>在这里</h3></div>';if(s.position!==p.id)body+=routeButton(p.id);else{
          const localActions={garage:actionCard('查看座驾','维修与升级','panel-vehicle','','bike'),clinic:actionCard('接受治疗','30 分钟 · ¥25 · 气血补满','heal','','shield',s.player.health>=c.health||s.player.money<25),temple:actionCard('在灵地修行','修炼收益额外提高 25%','panel-cultivation','','lotus')+actionCard('探寻古阵','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),park:actionCard('沿江探索','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),market:actionCard('吃一碗热面','20 分钟 · ¥12 · 体力 +45 / 气血 +10','meal','','tea',s.player.money<12)+actionCard('逛炼药摊','购买药鼎、药材并寻购丹方','panel-cultivation','','lotus')};
      ...
      case 'cultivation':{
        ...
        const need=G.realmNeed(s),nextRealm=G.nextRealmLabel(s),atMax=G.realmAtMax(s),foundationTier=G.highestPillTier(s,'foundation'),foundationCount=G.pillCount(s,'foundation'),chance=G.breakChance(s,usePill?foundationTier:0),alchemyRank=G.ALCHEMY_RANKS[G.alchemyRank(s)],cauldron=G.cauldron(s),atMarket=s.position==='market',pillCap=(s.alchemy?.cauldron||0)?Math.min(cauldron.tier,G.alchemistTier(s)):0,ownedCauldrons=Array.isArray(s.alchemy?.cauldrons)?s.alchemy.cauldrons:[],ownedRecipes=G.ALCHEMY_RECIPES.filter(r=>G.alchemyHasFormula(s,r)),formulaCandidates=G.formulaMarketCandidates(s),formulaCost=G.formulaScrollCost(s);
        if(!foundationCount)usePill=false;
        const cauldronGroups=G.CAULDRONS.slice(1).reduce((out,d)=>{(out[d.tier]||(out[d.tier]=[])).push(d);return out;},{});
        ...
        const materialGroups=G.ALCHEMY_MATERIALS.reduce((out,m)=>{(out[m.tier]||(out[m.tier]=[])).push(m);return out;},{});
