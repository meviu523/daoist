    ...
    switch(dialogType){
      ...
      case 'place':{
        ...
        if(p.permanent){body+='<div class="section-title"><h3>在这里</h3></div>';if(s.position!==p.id)body+=routeButton(p.id);else{
          const localActions={garage:actionCard('查看座驾','维修与升级','panel-vehicle','','bike'),clinic:actionCard('接受治疗','30 分钟 · ¥25 · 气血补满','heal','','shield',s.player.health>=c.health||s.player.money<25),temple:actionCard('在灵地修行','修炼收益额外提高 25%','panel-cultivation','','lotus')+actionCard('探寻古阵','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),park:actionCard('沿江探索','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),market:actionCard('吃一碗热面','20 分钟 · ¥12 · 体力 +45 / 气血 +10','meal','','tea',s.player.money<12)+actionCard('逛炼药摊','购买药鼎、药材并查看丹方','panel-cultivation','','lotus')};
      ...
      case 'cultivation':{
        ...
        const recipeGroups=[...new Set(G.ALCHEMY_RECIPES.map(r=>r.route))].map((route,groupIndex)=>{
          const cards=G.ALCHEMY_RECIPES.filter(r=>r.route===route).map(recipe=>{
            const unlocked=G.alchemyUnlocked(s,recipe),hasDing=(s.alchemy?.cauldron||0)>0,cauldronReady=G.alchemyCauldronReady(s,recipe),materialsReady=Object.entries(G.alchemyMaterials(recipe)).every(([id,count])=>(s.inventory[id]||0)>=count),ready=hasDing&&unlocked&&cauldronReady&&materialsReady&&s.player.mana>=recipe.mana,chanceText=hasDing?Math.floor(G.alchemyChance(s,recipe)*100)+'%':'需药鼎';
            let actionLabel='开炉炼制';if(!unlocked)actionLabel='尚未掌握';else if(!hasDing)actionLabel='先买药鼎';else if(!cauldronReady)actionLabel='药鼎阶位不足';else if(!materialsReady)actionLabel='药材不足';else if(s.player.mana<recipe.mana)actionLabel='灵力不足';
            return '<div class="card"><div class="row"><div><h3>'+esc(recipe.name)+'</h3><p class="copy">产物：'+esc(G.pillType(recipe.product).name)+' · 丹方复杂度 '+recipe.complexity+'</p></div>'+tag(chanceText,ready?'gold':'')+'</div><p class="copy">'+esc(G.alchemyMaterialText(recipe))+' · 灵力 '+recipe.mana+' · 火候 '+recipe.duration+' 分钟 · 至少 '+recipe.minCauldronTier+' 阶药鼎</p><p class="copy">成品阶数不由丹方固定；当前药鼎与炼药师水平的共同上限为 '+pillCap+' 阶。</p>'+btn(actionLabel,'data-act="alchemy" data-recipe="'+recipe.id+'"'+disabled(!ready),'small wide')+'</div>';
          }).join('');
          return '<details class="card"'+(groupIndex===0?' open':'')+'><summary><strong>'+esc(route)+'</strong> · 12 张丹方</summary><div class="stack" style="margin-top:12px">'+cards+'</div></details>';
        }).join('');
        ...
        body=`<div class="row"><h3>${G.realmLabel(s)}${nextRealm?` <span class="muted">→</span> ${nextRealm}`:''}</h3>${tag(G.isNight(s)?'夜间灵气 +45%':'白昼 · 灵气平缓',G.isNight(s)?'gold':'')}</div><div class="wide-progress"><i style="width:${need?Math.min(100,s.player.qi/need*100):100}%"></i></div><div class="progress-label"><span>修为</span><span>${Math.floor(s.player.qi)}${need?` / ${need}`:' · 化神九重圆满'}</span></div><div class="section-title"><h3>功课</h3>${s.position==='temple'?tag('灵地 +25%','mint'):tag(G.locationName(s))}</div><div class="stack">${actionCard('吐纳','45 分钟 · 体力 −12 · 恢复灵力','cultivate','data-kind="breath"','lotus',s.player.stamina<12)}${actionCard('静坐入定','90 分钟 · 体力 −24 · 更多修为','cultivate','data-kind="meditate"','moon',s.player.stamina<24)}${actionCard('淬体练步','30 分钟 · 体力 −20 · 积累根骨与身法','cultivate','data-kind="body"','sword',s.player.stamina<20)}</div>
          ...
          <div class="section-title"><h3>丹方</h3><span class="muted" style="font-size:11px">108 张 · 丹方与丹药分开</span></div><p class="copy" style="margin-bottom:12px">丹方决定药材配伍、火候和炼制难度；丹药的一至九阶由药鼎与炼药师共同决定，不由丹方写死。</p><div class="stack">${recipeGroups}</div>`;break;
      }
      case 'system':{
