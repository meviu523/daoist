(function(root){
  ...
  function persist(){if(!state)return;const r=store.upsert(state);saveOk=r.ok;storageWarning(r.ok?'':r.error);}
  ...
  function renderStart(){
    ...
    $('#start-screen').innerHTML=`<div class="start-top"><div class="brand"><span class="brand-mark">${icon('box')}</span>外卖修仙录</div><span class="tag">单机 · 重建版 1.0</span></div>
      <div class="start-shell"><section class="start-hero"><span class="eyebrow">A delivery. A destiny.</span><h1>人间一程<span class="gold-text">仙途万里</span></h1><p>白天，把热饭送到万家灯火。<br>入夜，在城市的另一面修行。<br>这一次，你想把故事送向哪里？</p>
      ...
      </section><section class="start-saves"><div class="row"><div><div class="eyebrow">Your journeys</div><h2 style="margin-top:5px">继续你的故事</h2></div><span class="tag">${saves.length} / ${G.MAX_SAVES}</span></div>
      ${btn(`${icon('spark')} 开始新的旅程`,'data-ui="new-game"','primary wide')}
      <div class="save-list">${saves.length?saves.map(s=>`<article class="save-card"><div class="row"><span class="save-name">${esc(s.name)}</span>${tag(G.REALMS[s.player.realm].name,'gold')}</div><div class="save-meta"><span>第 ${G.day(s)} 日 · ${G.clock(s)}</span><span>${s.stats.delivered} 单</span><span>${s.mode==='ai'?'AI 剧情':'经典剧情'}</span></div><div class="save-actions">${btn('继续旅程',`data-ui="load" data-id="${esc(s.id)}"`,'small soft')}<button class="icon-btn" data-ui="export" data-id="${esc(s.id)}" title="导出此存档" aria-label="导出${esc(s.name)}存档">${icon('download')}</button><button class="icon-btn" data-ui="delete" data-id="${esc(s.id)}" title="删除存档" aria-label="删除${esc(s.name)}存档">${icon('trash')}</button></div></article>`).join(''):`<div class="start-no-save"><div class="empty-symbol">启</div><p>还没有写下的故事，<br>正等你按下第一声门铃。</p></div>`}</div>
  ...
  function renderGame(){
    if(!state)return;const s=state,c=G.limits(s),realm=G.REALMS[s.player.realm];
    $('#start-screen').hidden=true;$('#game-screen').hidden=false;
    $('#game-header').innerHTML=`<div class="header-main"><button class="identity" data-panel="character" title="查看角色属性"><span class="avatar">${esc([...s.name][0])}</span><span><span class="player-name" style="display:block">${esc(s.name)}</span><small>${realm.name} · ${s.stats.delivered<10?'初行骑手':s.stats.delivered<30?'街巷熟客':'万家掌灯人'}</small></span></button>
      <div class="clock-wrap"><span class="weather">${icon(G.isNight(s)?'moon':'sun')}</span><div><div class="clock-time">${G.clock(s)}</div><div class="clock-desc">第 ${G.day(s)} 日 · ${G.WEATHER.find(w=>w.id===s.weather).name}</div></div></div>
    ...
    $('#map-caption').innerHTML=`<div class="city-name">青岚城</div><div class="eyebrow">Qinglan · ${G.isNight(s)?'After hours':'City of a thousand lights'}</div><button class="location-chip" data-panel="vehicle">${icon(s.transport==='bike'?'bike':'walk')} ${s.transport==='bike'?'骑行':'步行'} · ${esc(G.place(s.position).name)}</button>${next?`<button class="chapter-chip" data-ui="quests"><span>当前篇章</span><strong>${esc(next.title)}</strong><span>${esc(next.desc)} · ${Math.min(s.stats.delivered,next.delivery)}/${next.delivery}</span></button>`:`<button class="chapter-chip" data-ui="endings"><span>归途已近</span><strong>你想把故事送向哪里？</strong><span>查看可选择的结局</span></button>`}`;
    ...
    renderLogs();if(!map)map=new G.CityMap($('#city-map'),onPlace);map.render(s);
  }
  function renderLogs(){
  ...
  function onPlace(id){if(!state)return;if(state.pending){openPanel('event');return;}if(state.ending){openPanel('ending');return;}map.select(id);openPanel('place',{id});}
  function openPanel(type,data={}){dialogType=type;dialogData=data;clearError();renderPanel();dialog.classList.toggle('centered',!state||type==='new');if(!dialog.open)dialog.showModal();if(type==='new')setTimeout(()=>$('#player-name-input')?.focus(),40);}
  function closePanel(){if(state?.pending||state?.ending)return;dialog.close();dialogType=null;dialogData={};map?.select(null);clearError();}
  function panelTemplate(title,eyebrow,body,foot=''){
    const closable=!(state?.pending||state?.ending);
    return `<header class="panel-head"><div><div class="eyebrow">${eyebrow}</div><h2 id="panel-title">${esc(title)}</h2></div>${closable?`<button class="icon-btn" data-ui="close" aria-label="关闭面板">${icon('close')}</button>`:''}</header><div class="panel-body">${body}</div><div id="panel-error" class="panel-error" role="alert" hidden></div>${foot?`<footer class="panel-foot">${foot}</footer>`:''}`;
  }
  function travelLine(target){const p=G.travelPlan(state,target);return `<div class="trip-line"><span>沿路 <strong>${p.km.toFixed(2)} km</strong></span><span>行程 <strong>${p.minutes} 分钟</strong></span><span>体力 <strong>−${p.stamina}</strong></span>${p.transport==='bike'?`<span>电量 <strong>−${p.battery.toFixed(1)}</strong></span>`:''}</div>`;}
  ...
  function renderPanel(){
    ...
    switch(dialogType){
      ...
      case 'place':{
        ...
        for(const order of orders){const plan=G.travelPlan(s,p.id);let block=G.travelBlock(s,plan);if(!block&&s.minutes+plan.minutes>order.expiresAt)block='按当前速度，预计无法在时限内送达。';if(!block&&order.condition==='mystic'&&s.player.mana<8)block='灵异订单需要至少 8 点灵力。';if(!block&&order.condition==='careful'&&s.player.stamina-plan.stamina<10)block='易损餐品需要抵达时至少保留 10 点体力。';
          body+=`<article class="order-box"><div class="row"><h3>${esc(order.title)}</h3>${tag(`${order.expiresAt-s.minutes} 分钟内`,'gold')}</div><p class="copy" style="margin-top:10px">${esc(order.desc)}</p><div class="order-reward"><span>¥${order.reward}<small> 现金</small></span><span>${order.coins}<small> 外卖币</small></span></div>${travelLine(p.id)}${order.condition==='mystic'?'<p class="copy">交付需要灵力 −8。</p>':''}${block?`<div class="alert" style="margin:12px 0">${esc(block)}</div>`:''}${btn('接单并配送 →',`data-act="deliver" data-id="${esc(order.id)}"${disabled(!!block)}`,'primary wide')}</article>`;
        }
        if(p.permanent){body+='<div class="section-title"><h3>在这里</h3></div>';if(s.position!==p.id)body+=routeButton(p.id);else{
          const localActions={home:actionCard('回屋睡一觉','8 小时 · 恢复体力、灵力与电量','sleep','','moon')+actionCard('在小屋静修','进入修行面板','panel-cultivation','','lotus'),garage:actionCard('查看座驾','充电、维修与升级','panel-vehicle','','bike'),clinic:actionCard('接受治疗','30 分钟 · ¥25 · 气血补满','heal','','shield',s.player.health>=c.health||s.player.money<25),temple:actionCard('在灵地修行','修炼收益额外提高 25%','panel-cultivation','','lotus')+actionCard('探寻古阵','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),park:actionCard('沿江探索','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),market:actionCard('吃一碗热面','20 分钟 · ¥12 · 体力 +45 / 气血 +10','meal','','tea',s.player.money<12)+actionCard('买一株青灵草','¥15 · 炼丹材料','herb','','bag',s.player.money<15)};
          body+=`<div class="stack">${localActions[p.id]||''}</div>`;
        }
        const npc=G.NPCS.find(n=>n.place===p.id);if(npc)body+=`<div style="margin-top:12px">${btn(`拜访${esc(npc.name)}`,`data-act="visit" data-id="${npc.id}"${disabled(s.bonds[npc.id].lastTalkDay===G.day(s))}`,'wide')}</div>`;
      ...
      case 'character':{
        ...
        body=`<div class="row">${tag(`${G.REALMS[s.player.realm].name} · ${G.place(s.position).name}`,'gold')}${tag(s.mode==='ai'?'AI 剧情':'经典剧情')}</div><div class="stat-grid" style="margin-top:20px">${statsBox('气血',captext(s.player.health,c.health))}${statsBox('体力',captext(s.player.stamina,c.stamina))}${statsBox('灵力',captext(s.player.mana,c.mana))}</div><div class="section-title"><h3>修行资质</h3></div><div class="detail-list">${Object.entries(labels).map(([k,n])=>`<div><span>${n}</span><strong>${s.player[k]}</strong></div>`).join('')}</div><div class="section-title"><h3>沿途留下的印记</h3></div><div class="stat-grid">${statsBox('送达订单',s.stats.delivered,'单')}${statsBox('累计行程',(s.stats.distance/1000).toFixed(1),'km')}${statsBox('配送收入',s.stats.earned,'元')}</div><p class="copy" style="margin:17px 0">已习得：${s.learned.map(id=>G.ITEMS.find(i=>i.id===id).name).join('、')||'暂无心法'}<br>已装备：${s.equipment.map(id=>G.ITEMS.find(i=>i.id===id).name).join('、')||'暂无法器'}</p>${btn(`${icon('download')} 导出当前存档`,'data-ui="export-current"','wide')}`;break;
      }
      case 'cultivation':{
        title='今夜，向内而行';label='Cultivation';const r=G.REALMS[s.player.realm],chance=G.breakChance(s,usePill);
        body=`<div class="row"><h3>${r.name}${r.need?` <span class="muted">→</span> ${G.REALMS[s.player.realm+1].name}`:''}</h3>${tag(G.isNight(s)?'夜间灵气 +45%':'白昼 · 灵气平缓',G.isNight(s)?'gold':'')}</div><div class="wide-progress"><i style="width:${r.need?Math.min(100,s.player.qi/r.need*100):100}%"></i></div><div class="progress-label"><span>修为</span><span>${s.player.qi}${r.need?` / ${r.need}`:' · 已达化神'}</span></div><div class="section-title"><h3>功课</h3>${s.position==='temple'?tag('灵地 +25%','mint'):tag(G.place(s.position).name)}</div><div class="stack">${actionCard('吐纳','45 分钟 · 体力 −12 · 恢复灵力','cultivate','data-kind="breath"','lotus',s.player.stamina<12)}${actionCard('静坐入定','90 分钟 · 体力 −24 · 更多修为','cultivate','data-kind="meditate"','moon',s.player.stamina<24)}${actionCard('淬体练步','30 分钟 · 体力 −20 · 积累根骨与身法','cultivate','data-kind="body"','sword',s.player.stamina<20)}</div>
          <div class="section-title"><h3>叩问下一重天</h3>${r.need?tag(`成功率 ${Math.floor(chance)}%`,'gold'):tag('道心已明')}</div>${r.need?`<label class="checkbox"><input type="checkbox" id="break-pill"${usePill?' checked':''}${disabled(!(s.inventory.foundation>0))}>使用破境丹（现有 ${s.inventory.foundation||0} 枚，成功率 +15%）</label><p class="copy" style="margin:11px 0">${(s.inventory.charm||0)>0?'背包中的护身符会在失败时自动保护你。':'失败会损失本阶需求的 25% 修为与 18 气血。'}突破耗时 60 分钟、消耗 20 体力。</p>${btn('尝试突破',`data-act="breakthrough"${disabled(s.player.qi<r.need||s.player.stamina<20)}`,'primary wide')}`:btn('看看自己的归途','data-ui="endings"','primary wide')}
          <div class="section-title"><h3>丹炉</h3><span class="muted" style="font-size:11px">灵草 ${s.inventory.herb||0}</span></div><div class="button-row">${btn('凝气丹 · 2 草 / 10 灵力',`data-act="alchemy" data-recipe="qi"${disabled((s.inventory.herb||0)<2||s.player.mana<10)}`,'small')}${btn('回春丹 · 1 草 / 8 灵力',`data-act="alchemy" data-recipe="heal"${disabled((s.inventory.herb||0)<1||s.player.mana<8)}`,'small')}</div>`;break;
      }
      case 'system':{
        title='万家灯火系统';label='The lantern system';body=`<div class="row" style="margin-bottom:20px"><span class="gold-text" style="font-size:22px">${s.player.coins} <small style="font-size:11px">外卖币</small></span>${btn(s.daily.signedDay===G.day(s)?'今日已签到':'每日签到',`data-act="sign"${disabled(s.daily.signedDay===G.day(s))}`,'small soft')}</div><div class="tabs" role="tablist" aria-label="系统分类">${[['shop','兑换'],['quests','委托'],['endings','归途']].map(([id,n])=>`<button role="tab" aria-selected="${id===systemTab}" class="${id===systemTab?'active':''}" data-ui="system-tab" data-id="${id}">${n}</button>`).join('')}</div>`;
        if(systemTab==='shop'){
          body+=`<div class="shop-grid">${G.ITEMS.map(item=>{const owned=item.unique&&(s.learned.includes(item.id)||s.equipment.includes(item.id));return `<article class="shop-item"><span class="item-glyph">${item.glyph}</span><h3>${item.name}</h3><p>${item.desc}</p>${btn(owned?'已拥有':`${item.cost} 币 · 兑换`,`data-act="buy" data-id="${item.id}"${disabled(owned||s.player.coins<item.cost)}`,'small')}</article>`;}).join('')}</div>`;foot='远程即时兑换 · 无需前往驿站 · 不消耗游戏时间';
      ...
      case 'bonds':{
        title='灯火里的故人';label='People you meet';body=`<div class="stack">${G.NPCS.map(n=>{const b=s.bonds[n.id],travel=G.travelPlan(s,n.place),blocked=b.lastTalkDay===G.day(s)||!!G.travelBlock(s,travel);return `<article class="npc-card" style="--npc-color:${n.color}"><div class="row"><div class="npc-avatar">${n.name[0]}</div><div><h3>${n.name}</h3><small>${n.role} · ${G.place(n.place).name}</small></div><span class="tag" style="margin-left:auto">${b.path==='romance'?'相伴':b.stage>=4?'知己':b.stage>0?'相识':'初见'}</span></div><p>${n.bio}</p><div class="progress-label"><span>好感 ${b.affinity} · 信任 ${b.trust}</span><span>故事 ${b.stage}/4</span></div><div class="wide-progress"><i style="width:${b.affinity}%"></i></div><div class="npc-footer"><small>行程 ${travel.minutes} 分钟 · 交谈 20 分钟</small>${btn(b.lastTalkDay===G.day(s)?'明天再见':'去见一面',`data-act="visit" data-id="${n.id}"${disabled(blocked)}`,'small')}</div>${blocked&&b.lastTalkDay!==G.day(s)?`<p class="copy">${esc(G.travelBlock(s,travel))}</p>`:''}</article>`;}).join('')}</div>`;foot='人物关系由选择推进。友情与情感路线不会自动替你决定。';break;
      }
      case 'vehicle':{
        title='陪你走过每一程';label='Your electric companion';const atGarage=s.position==='garage',canCharge=['home','garage'].includes(s.position),repairCost=Math.ceil((c.durability-s.vehicle.durability)*.6)+12;
        body=`<div class="tabs" role="group" aria-label="出行方式"><button class="${s.transport==='bike'?'active':''}" data-act="transport" data-mode="bike">电动车骑行</button><button class="${s.transport==='walk'?'active':''}" data-act="transport" data-mode="walk">步行推车</button></div><div class="stat-grid">${statsBox('基础车速',22+s.vehicle.levels.speed*4,'km/h')}${statsBox('剩余电量',Math.floor(s.vehicle.battery),`/ ${c.battery}`)}${statsBox('当前车况',Math.floor(s.vehicle.durability),`/ ${c.durability}`)}</div>${s.vehicle.battery<15?'<div class="alert" style="margin-top:15px">电量偏低。无法骑行时可切换步行，到家或修车铺充电。</div>':''}<div class="section-title"><h3>保养</h3>${tag(canCharge?'可充电':'需到家或修车铺')}</div><div class="button-row">${btn('充电 · ¥8 / 30 分钟',`data-act="charge"${disabled(!canCharge||s.vehicle.battery>=c.battery-.01||s.player.money<8)}`,'small')}${btn(`维修 · ¥${repairCost}`,`data-act="repair"${disabled(!atGarage||s.vehicle.durability>=c.durability-.01||s.player.money<repairCost)}`,'small')}</div><div class="section-title"><h3>升级</h3>${tag('每项最高 Lv.5')}</div><div class="stack">${[['speed','速度','每级 +4 km/h'],['battery','电池容量','每级 +35 容量'],['durability','耐用性','每级 +30 上限，并降低磨损']].map(([id,n,desc])=>`<div class="card"><div class="row"><div><h3>${n} <span class="muted">Lv.${s.vehicle.levels[id]}</span></h3><p class="copy">${desc}</p></div>${btn(s.vehicle.levels[id]>=5?'已满级':`¥${G.upgradeCost(s,id)}`,`data-act="upgrade" data-kind="${id}"${disabled(!atGarage||s.vehicle.levels[id]>=5||s.player.money<G.upgradeCost(s,id))}`,'small')}</div></div>`).join('')}</div>${!atGarage?`<div class="section-title"><h3>阿默修车铺</h3></div>${routeButton('garage','前往修车铺')}`:''}`;foot='升级在修车铺进行，每项耗时 45 分钟。步行不会耗电；空电或车况归零时禁止骑行。';break;
      }
      case 'rest':{
        title='先照顾好自己';label='Take a breath';const plan=G.travelPlan(s,'home'),block=G.travelBlock(s,plan);
        body=`<p class="copy" style="margin-bottom:20px">${G.place(s.position).name} · ${G.clock(s)}</p><div class="stack">${actionCard('原地休息','60 分钟 · 体力 +38 / 气血 +8 / 灵力 +10','rest','','tea')}${actionCard('回家睡个好觉',`路程 ${plan.minutes} 分钟 + 8 小时 · 体力、灵力、电量补满`,'sleep','','moon',!!block)}</div>${block?`<p class="alert" style="margin-top:14px">${esc(block)}</p>`:''}<div class="section-title"><h3>身上的补给</h3></div><div class="button-row">${btn(`清心散 ×${s.inventory.stamina||0}`,`data-act="use" data-id="stamina"${disabled(!(s.inventory.stamina>0)||s.player.stamina>=c.stamina)}`,'small')}${btn(`回春丹 ×${s.inventory.heal||0}`,`data-act="use" data-id="heal"${disabled(!(s.inventory.heal>0)||s.player.health>=c.health)}`,'small')}</div>`;foot='时间只随行动推进。看地图、阅读说明、兑换系统物品都不会让你错过夜晚。';break;
      }
      case 'event':{
      ...
      case 'help':{
        title='给初来此城的你';label='Field notes';body=`<div class="help-section"><h3>白天送外卖，入夜修行</h3><p>点地图上的金色订单标记，查看要求、距离、耗时与报酬，再选择「接单并配送」。取餐与送餐已经合并，没有单独的取餐流程。送达后角色移动到目的地，成功行动后刷新配送点；普通收餐地点仅在有订单时显示。</p></div><div class="help-section"><h3>时间与地图</h3><p>这是行动推进时间的单机 RPG，不会因你挂机或关掉浏览器而自动流逝。移动时间 = 道路距离 ÷ 当前速度，向上取整到分钟。过江必须走桥，雨雾会影响速度。修炼、交谈、充电、休息有独立活动时长；需要出行的活动再叠加路程。系统兑换、签到、领奖、用药不耗时。</p><p>鼠标滚轮缩放，按住拖动平移；手机可单指平移、双指缩放。键盘方向键平移，+ / − 缩放，Home 查看全城。右上角「◎」定位玩家。</p></div><div class="help-section"><h3>系统与修仙</h3><p>外卖币从成功配送、签到与委托奖励中获得。直接打开系统兑换，不必跑驿站。丹药在行囊使用，心法与装备兑换后立即生效。18:00–次日 06:00 修炼有夜间加成；听雨观另有灵地加成。修为达到门槛即可突破，失败会有损失。炼丹需要灵草与灵力。</p></div><div class="help-section"><h3>电动车与休息</h3><p>电量不足或车况归零会阻止骑行，但可以切换步行推车。回小屋睡眠会充满电量；小屋与修车铺均可花钱充电。速度、电池容量、耐用性在修车铺独立升级，各最高 5 级。每七天扣除一次房租，最多 ¥120；不足部分暂免，不会形成死局。</p></div><div class="help-section"><h3>角色与人际关系</h3><p>悟性影响修炼、炼丹与突破，根骨降低移动体力消耗，身法影响步行速度；机缘影响随机奇遇触发率，以及探索时额外发现灵草的概率。善缘与口碑影响路线条件和配送收益。四位 NPC 各有四章故事，每日最多深入交谈一次。友情与情感路线需你明确选择，不会自动绑定。五种结局都允许继续游玩。</p></div><div class="help-section"><h3>剧情与日志</h3><p>行为与事件结果只记录在右下角「行旅日志」。事件弹窗用于当前选择，不另建历史记录。经典模式无需网络；AI 模式生成随机奇遇，关键主线与人物章节保持固定。AI 的效果由引擎白名单校验，超限、无免费选项或接口失败都会回退经典事件。</p></div><div class="help-section"><h3>AI 接入</h3><p>当前：${esc(aiHealth.message)}。在项目根目录按 <code>.env.example</code> 配置服务端地址、模型和密钥，用 <code>npm start</code> 启动。密钥不进入浏览器或存档。AI 模式会将角色名、部分状态、地点与最近剧情摘要发送到你配置的服务，可能产生调用费用。模式只能在新建存档时选择，局内不可更改。</p></div><div class="help-section"><h3>自动存档与备份</h3><p>最多保存 ${G.MAX_SAVES} 段旅程，每次成功行动立即自动保存，另外保留上一次写入前的本地备份。导入一律生成新副本，不覆盖已有进度。浏览器清理网站数据会删除本地存档，请定期导出。多个标签页同时操作会暂停旧页，避免覆盖新进度。纯单机数据可被开发者工具修改；本版不提供服务端防作弊。</p><p>此版本按对话中的需求重新实现，不是原仓库逐文件恢复。原始存档结构未知，不保证能够直接导入；仅对重建版已识别结构做升级处理。</p></div><div class="button-row">${btn('导入存档','data-ui="import"','small')}${btn('导出全部','data-ui="export-all"','small')}${btn('恢复上次自动备份','data-ui="restore"','small')}</div>`;break;
      }
      default:return;
    }
    $('#panel-content').innerHTML=panelTemplate(title,label,body,foot);
  }
  function loadGame(id){const found=store.saves.find(s=>s.id===id);if(!found)return showError('没有找到这个存档。');abortAI();state=G.clone(found);conflict=false;saveOk=!store.unavailable;dialog.close();dialogType=null;clearError();renderGame();requestAnimationFrame(()=>{if(window.innerWidth<800)map.center();else map.fit();});if(state.pending){openPanel('event');maybeGenerateAI();}else if(state.ending)openPanel('ending');}
  function act(action,payload={}){
    ...
    if(state.pending){openPanel('event');maybeGenerateAI();}
    else if(state.ending)openPanel('ending');
    else if(['deliver','travel','visit','choose','sleep','continue'].includes(action)){closePanel();}
  ...
  async function maybeGenerateAI(){
    ...
    try{
      const context={name:s.name,realm:G.REALMS[s.player.realm].name,day:G.day(s),time:G.clock(s),weather:s.weather,location:G.place(s.position).name,kind:p.kind,title:p.title,text:p.text,npc:p.npcId?G.NPCS.find(n=>n.id===p.npcId).name:null,stats:{stamina:s.player.stamina,mana:s.player.mana,money:s.player.money,qi:s.player.qi,karma:s.player.karma,rep:s.player.rep},recent:s.logs.slice(-3).map(l=>l.text)};
  ...
  function returnHome(){abortAI();state=null;dialog.close();dialogType=null;dialogData={};conflict=false;clearError();store.load();renderStart();}
  document.addEventListener('click',e=>{
    const button=e.target.closest('button');if(!button||button.disabled)return;
    if(button.dataset.panel){if(state?.pending)openPanel('event');else if(state?.ending)openPanel('ending');else openPanel(button.dataset.panel);return;}
    ...
    switch(d.ui){
      ...
      case 'export-all':{let saves=store.saves;if(state&&!saves.some(s=>s.id===state.id))saves=[state,...saves];download(saves);break;}
  ...
  document.addEventListener('submit',e=>{
    ...
    try{const data=new FormData(e.target),newState=G.newGame(data.get('name'),data.get('mode'));const result=store.upsert(newState);if(!result.ok&&store.saves.length>=G.MAX_SAVES&&!store.saves.some(s=>s.id===newState.id))throw new Error(result.error);saveOk=result.ok;if(!result.ok)storageWarning(result.error);loadGame(newState.id);}catch(err){showError(err.message);}
  });
  document.addEventListener('change',e=>{if(e.target.id==='break-pill'){usePill=e.target.checked;renderPanel();}});
  ...
  dialog.addEventListener('cancel',e=>{if(state?.pending||state?.ending)e.preventDefault();else{dialogType=null;map?.select(null);}});
  ...
  window.addEventListener('storage',e=>{if(e.key===G.STORAGE_KEY){if(state){conflict=true;abortAI();storageWarning('另一标签页已更新存档，本页已暂停行动。请返回开始页重新载入。');}else{store.load();renderStart();}}});
  renderStart();checkAI();
})(globalThis);
