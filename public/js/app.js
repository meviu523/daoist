(function(root){
  ...
  function renderGame(){
    ...
    $('#game-nav').innerHTML=[['cultivation','lotus','修行'],['alchemy','tea','炼药'],['system','spark','系统'],['inventory','bag','行囊'],['bonds','people','羁绊'],['vehicle','bike','座驾'],['rest','tea','歇息']].filter(([p])=>G.panelUnlocked(s,p)).map(([p,i,l])=>`<button class="nav-btn ${p==='system'?'featured':''}" data-panel="${p}">${icon(i)}<span>${l}</span></button>`).join('')+`<span class="nav-sep"></span><button class="nav-btn" data-panel="help">${icon('help')}<span>帮助</span></button>`;
  ...
  function renderPanel(){
      ...
      case 'bonds':{
        ...
        if(!known.length)body='<div class="empty">旅途才刚开始。完成配送、经历奇遇，会逐渐认识这座城里的人。</div>';
          ...
          const cards=known.map(n=>{
            const b=s.bonds[n.id],transmission=G.formulaBondProgress(s,n.id),travel=G.travelPlan(s,n.place),blocked=b.lastTalkDay===G.day(s)||!!G.travelBlock(s,travel);
    ...
    if(s?.activity)for(const button of dialog.querySelectorAll('[data-act]')){
      if(!['buy','sign','claim','stop','panel-cultivation','panel-alchemy','panel-vehicle'].includes(button.dataset.act))button.disabled=true;
    }
  }
  ...
  function act(action,payload={}){
    if(!state)return;if(['panel-cultivation','panel-alchemy','panel-vehicle'].includes(action)){openPanel(action.slice(6));return;}
