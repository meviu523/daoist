(function(root){
  ...
  function renderStart(){
    ...
    $('#start-screen').innerHTML=`<div class="start-top"><div class="brand"><span class="brand-mark">${icon('box')}</span>外卖修仙录</div><span class="tag">单机 · 连续时间版 1.3</span></div>
  ...
  function renderPanel(){
    ...
    switch(dialogType){
      ...
      case 'cultivation':{
        title='今夜，向内而行';label='Cultivation';const r=G.REALMS[s.player.realm],need=G.realmNeed(s),nextRealm=G.nextRealmLabel(s),atMax=G.realmAtMax(s),chance=G.breakChance(s,usePill),alchemyRank=G.ALCHEMY_RANKS[G.alchemyRank(s)],cauldron=G.cauldron(s),nextCauldron=G.CAULDRONS[(s.alchemy?.cauldron||0)+1],atMarket=s.position==='market';
