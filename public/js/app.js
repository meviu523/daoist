    ...
    switch(dialogType){
      ...
      case 'place':{
        ...
        if(p.permanent){body+='<div class="section-title"><h3>在这里</h3></div>';if(s.position!==p.id)body+=routeButton(p.id);else{
          const localActions={garage:actionCard('查看座驾','维修与升级','panel-vehicle','','bike'),clinic:actionCard('接受治疗','30 分钟 · ¥25 · 气血补满','heal','','shield',s.player.health>=c.health||s.player.money<25),temple:actionCard('在灵地修行','修炼收益额外提高 25%','panel-cultivation','','lotus')+actionCard('探寻古阵','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),park:actionCard('沿江探索','30 分钟 · 体力 −12','explore','','spark',s.player.stamina<12),market:actionCard('吃一碗热面','20 分钟 · ¥12 · 体力 +45 / 气血 +10','meal','','tea',s.player.money<12)+actionCard('买一株青灵草','¥15 · 炼药材料','herb','','bag',s.player.money<15)+actionCard('逛炼药摊','购买药鼎、查看丹方与炼药熟练度','panel-cultivation','','spark')};
      ...
      case 'cultivation':{
        title='今夜，向内而行';label='Cultivation';const r=G.REALMS[s.player.realm],chance=G.breakChance(s,usePill),alchemyRank=G.ALCHEMY_RANKS[G.alchemyRank(s)],cauldron=G.cauldron(s),nextCauldron=G.CAULDRONS[(s.alchemy?.cauldron||0)+1],atMarket=s.position==='market';
