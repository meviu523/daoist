(function(root){
  ...
  function openPanel(type,data={}){dialogType=type;dialogData=data;clearError();renderPanel();dialog.classList.toggle('centered',!state||type==='new'||type==='gameover');if(!dialog.open)dialog.showModal();if(type==='new')setTimeout(()=>$('#player-name-input')?.focus(),40);}
  function closePanel(){if(state?.gameOver||state?.pending||state?.ending)return;dialog.close();dialogType=null;dialogData={};map?.select(null);clearError();}
  ...
  function act(action,payload={}){
    ...
    else if(['deliver','travel','visit','choose','sleep','moveHome','continue'].includes(action)){closePanel();}
