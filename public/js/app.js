(function(root){
  ...
  function openPanel(type,data={}){if(state&&!G.panelUnlocked(state,type)){const f=G.FEATURE_UNLOCKS.find(f=>f.id===G.FEATURE_PANELS[type]);showError(`${f.name}尚未开启：${f.hint}。`);return;}clock.pause('panel');if(state)persist();dialogType=type;dialogData=data;clearError();renderPanel();dialog.classList.toggle('centered',!state||type==='new'||type==='gameover');if(!dialog.open)dialog.showModal();renderLive();if(type==='new')setTimeout(()=>$('#player-name-input')?.focus(),40);}
  ...
  document.addEventListener('click',e=>{
    ...
    switch(d.ui){
      ...
      case 'endings':if(!state||!G.featureUnlocked(state,'endings'))break;systemTab='endings';if(state?.pending)openPanel('event');else openPanel('system');break;
