...
test('扩图前 v5 途中存档可恢复，不移动位置或重新扣费',()=>{
  ...
  for(const key of ['money','coins','health','stamina','mana','qi','realm'])near(s.player[key],before.player[key]);assert.equal(s.player.realmLevel,1);assert.deepEqual(s.vehicle,before.vehicle);
});
