...
test('真实新档只拥有起始住处；地点和住宅查询不修改状态',()=>{
  const s=fresh(),before=JSON.stringify(s);assert.deepEqual(s.unlockedPlaces,['home']);
  assert.deepEqual(G.visibleServicePlaces(s).map(p=>p.id),['home']);assert.deepEqual(G.visibleResidences(s).map(h=>h.id),['qingteng']);
...
test('有订单也不能未经接单自由前往未知地点，失败不花钱、不改种子与时钟',()=>{
  ...
  reject(s,'moveHome',{id:'qiyun'});s.bonds.chen.met=true;reject(s,'visit',{id:'chen'});
});

test('接单、途中保存、停止和取消都不授予地点',()=>{
  let s=fresh();const o=ticket(s);s=act(s,'deliver',{id:o.id});assert.deepEqual(s.unlockedPlaces,['home']);
  s=G.advance(s,.3);s=read(s);assert.deepEqual(s.unlockedPlaces,['home']);
  s=act(s,'stop');s=act(s,'cancelDelivery');assert.deepEqual(s.unlockedPlaces,['home']);
...
test('首次抵达和待选读档仍未解锁，实际结算后才解锁且保留原奖励',()=>{
  ...
  reject(s,'choose',{eventId:oldEvent,index:0});assert.deepEqual(initial.unlockedPlaces,['home']);
});
...
test('地图待接单消失不会变成地点解锁',()=>{
  ...
  assert.deepEqual(s.unlockedPlaces,['home']);assert.equal(discoveryLogs(s).length,0);
});

for(const p of G.PLACES.filter(p=>p.permanent&&p.id!=='home'))test(`送达${p.name}后开放常驻入口，路网与坐标不变`,()=>{
...
test('仅功能已开放不能绕过地点检查；所有本地服务都原子拒绝',()=>{
  ...
  for(const [place,action,p] of [['garage','repair',{}],['garage','upgrade',{kind:'speed'}],['clinic','heal',{}],['park','explore',{}],['market','meal',{}],['market','cauldron',{id:1}],['market','formula',{}],['market','herb',{}],['market','material',{id:'herb'}],['market','auctionCatalog',{}],['market','auctionBid',{id:'invalid'}]]){
...
test('相逢人物、提升关系、领取功能奖励不会间接授予其地点',()=>{
  let s=fresh();s.bonds.chen.met=true;s.bonds.chen.affinity=80;s.stats.delivered=10;s.claimed=['q1'];
  s=act(s,'transport',{mode:'walk'});assert.ok(G.panelUnlocked(s,'bonds'));assert.equal(G.placeUnlocked(s,'garage'),false);
  assert.match(reject(s,'visit',{id:'chen'}),/尚未解锁/);assert.deepEqual(s.unlockedPlaces,['home']);
});
...
test('AI文本不能授予地点；只有保留下来的配送结算可授予真实目标',()=>{
  ...
  assert.deepEqual(s.unlockedPlaces,['home']);s=choose(s);assert.ok(G.placeUnlocked(s,'market'));assert.equal(G.placeUnlocked(s,'temple'),false);
});
...
test('v12持久化严格校验地点列表，去重但不猜测授予未知地点',()=>{
  ...
  const s=fresh();s.unlockedPlaces=['home','home','market'];assert.deepEqual(read(s).unlockedPlaces,['home','market']);
  s.residenceId='qiyun';assert.throws(()=>read(s),/住处/);
});

test('新格式读档不能通过在途拜访、搬家、赶路或当地工作跳过地点权限',()=>{
  for(const [place,kind,params] of [['market','travel',{target:'market'}],['stop-3','moveHome',{id:'qiyun'}],['garage','visit',{id:'chen'}]]){
    let s=fresh();s.unlockedPlaces.push(place);s.bonds.chen.met=true;s=act(s,kind,params);s.unlockedPlaces=['home'];assert.throws(()=>read(s),/地点权限/);
  }
  let s=fresh();s.position='clinic';s.unlockedPlaces.push('clinic');s.player.health=10;s=act(s,'heal');s.unlockedPlaces=['home'];assert.throws(()=>read(s),/地点权限/);
});
...
test('旧本地键迁移到v12且不跨存档串通地点，新档仍只开放小屋',()=>{
  ...
  const store=G.createStore(storage),[a,b]=store.load();assert.ok(G.placeUnlocked(a,'market'));assert.deepEqual(b.unlockedPlaces,['home']);
