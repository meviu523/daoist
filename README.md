...
这是根据项目对话记录重新实现的单机 Web RPG，版本 **1.2.1**，重建日期 **2026-09-18**。原账号读取返回 `403: Sorry. Your account was suspended`，没有取得旧仓库源码。因此，这是**玩法与需求重建**，不是原始代码、界面和剧情文本的逐字恢复。
...
地图继续向南扩为 **11×10 / 110 个地点**，世界尺寸 **2660×1860**，面积约最初 42 地点版本的 **2.55 倍**。保留 1.2.0 已发布的全部 99 个地点、四座桥及最短路线，新添南岸新街 11 个动态配送地址；普通地点仍只随订单出现。10 分钟 / ¥8 原地充电及旧充电存档按比例迁移保持不变。

切换标签页或应用不再自动暂停。同一会话由统一时钟在前台和后台按所选倍率推进，延迟回调分批处理；手动暂停、面板、剧情、存档冲突和游戏结束仍停止计时，不替玩家做剧情选择。关闭/重载页面不补算离线时间，读档默认暂停；浏览器或操作系统可能冻结/销毁页面，不承诺后台保活。

本地 176 项 Node 测试与 25 组 Chromium 检查通过，包含 99 地点全部 9801 对路径兼容、10 分钟充电回归及无动画回调时的真实后台心跳。
...
| 动态世界 | 每 15 游戏分钟及世界行动完成后补充订单空缺，已接单不被刷新删除；有昼夜、天气、每日委托与每周房租。玩家可在三处住处间搬家，租金与睡眠恢复不同；房租到期现金不足会结束本局。运行时即使不操作也会计时；全局暂停不计时，页面后台继续计时；关闭/重载后不补算离线时间。 |
...
面板和剧情自动暂停，关闭后恢复打开前的状态；不会解除手动暂停。切换标签页或应用不会自动暂停，后台按当前倍率继续计时；页面生命周期变化仍自动保存。浏览器延迟后台计时器时，仍存活的会话会分批推进经过的时间，遇到剧情选择立即停下。要暂时离开而不计时，请先手动暂停。真正关闭或重载后没有离线补算，读档仍需手动继续；不保证浏览器或操作系统保持后台页面运行。睡眠、充电、休息、修炼按进度恢复/增长；炼丹和突破开始时扣投入，中断不退，突破正常失败会退回约 75% 修为。耗时剧情选择提交后不可取消。
...
扩图与后台继续版本本地验证中，**176 项 Node 规则/存档/本机 HTTP 测试通过，25 组离线 Chromium 交互检查通过**，并成功构建。保留快速充电、新区配送和旧路径兼容测试；新增同一会话后台计时、待选事件/房租边界、时间戳去重与长间隔分批推进。浏览器测试包含真实 requestAnimationFrame 与禁用动画回调后的真实 setInterval 探针；隐藏属性、存储及 AI 显式模拟，不代表物理手机锁屏保活验证。
