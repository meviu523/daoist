...
这是根据项目对话记录重新实现的单机 Web RPG，版本 **1.0.0**，重建日期 **2026-09-18**。原账号读取返回 `403: Sorry. Your account was suspended`，没有取得旧仓库源码。因此，这是**玩法与需求重建**，不是原始代码、界面和剧情文本的逐字恢复。
...
| 模块 | 当前实现 |
| --- | --- |
| 地图配送 | 青岚城 42 个地点，包含 7 个常驻服务地点与 35 个潜在配送目的地；服务地点也可能收到订单。普通配送点仅在有任务时显示。 |
| 合并配送流程 | 点击地图任务点查看要求、距离、预计耗时、电量与体力，确认后直接配送，不另设取餐流程。 |
| 位置与时间 | 玩家有真实地图位置；沿道路寻路，跨江走桥；出行耗时由路程、交通方式、速度、天气及车况计算。 |
| 动态世界 | 成功执行世界行动后刷新订单；有昼夜、天气、每日委托与每周生活支出。浏览器闲置不会让游戏时间自行流逝。 |
| 多选项事件 | 17 个原创随机事件模板，通常三选一；时间、金钱、资源、善缘、口碑等改变会参与后续结算。 |
| 系统兑换 | 万家灯火系统；配送获得外卖币，远程兑换丹药、材料、心法和装备；兑换、签到、领奖均不耗游戏时间。 |
| 修仙 | 六个境界，吐纳、静修、淬体、突破、炼丹、心法和法器；夜间与灵地加成，突破存在失败与保护机制。 |
| 电动车 | 保留电量、车况、充电、维修；速度、电池容量、耐用性可分别升级。空电或电量不足时不能骑行，可以改步行。 |
| 人物关系 | 4 位原创 NPC、共 16 个分支章节；好感、信任、友情与可选情感路线，不强制进入恋爱。 |
| 结局 | 守城、飞升、烟火生活、情感与友情共 5 类结局；达成后可以继续游玩。 |
| 存档 | 自定义姓名，最多 12 份独立存档，逐行动自动保存，JSON 导入/导出，上一写入版本备份，损坏检测。 |
...
现金用来维持生活、充电、修车与升级；外卖币直接在左下角“系统”兑换物品。体力不足可在“歇息”原地恢复，也可回小屋睡觉。没有电时，在“座驾”切换步行；不要强行接下无法完成的行程。
...
当前格式是 `schemaVersion: 3`。迁移逻辑只支持能够识别的重建版 v1/v2/v3 字段，**没有验证过原仓库旧存档格式**，不能保证旧存档可直接导入。不要为了通过校验，简单把未知旧存档的版本号改成 3。
...
交付时，**87 项 Node 规则/存档/本机 HTTP 测试通过，11 项离线浏览器交互检查通过**。自动连续游玩覆盖了 40 单配送、修为推进与守城结局可达性。

浏览器检查使用 Chromium 渲染真实构建页面，但受环境导航限制，存储和 AI 响应使用模拟实现；本机 HTTP 路由另行用真实 HTTP 测试。未验证真实付费模型、物理手机、Safari/Firefox、真实 `file:` 来源存档持久化或生产部署。具体证据与局限见 `docs/TEST_REPORT.md`。

这是本地单机游戏，玩家能够使用开发者工具修改自身数据。数值校验用于避免坏档和限制 AI 影响，**不是防作弊或服务端权威存档**。原提交历史、原账号、原仓库权限及旧部署没有恢复，也没有向任何远程仓库推送。
