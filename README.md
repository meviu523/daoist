...
面板和剧情自动暂停，关闭后恢复打开前的状态；不会解除手动暂停。切后台或锁屏不会新增手动暂停；浏览器后台停止动画的时间不补算，返回后自动恢复离开前的运行/暂停状态。读档仍默认暂停。睡眠、充电、休息、修炼按进度恢复/增长；炼丹和突破开始时扣投入，中断不退，突破正常失败会退回约 75% 修为。耗时剧情选择提交后不可取消。
