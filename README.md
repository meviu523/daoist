...
| 动态世界 | 成功执行世界行动后刷新订单；有昼夜、天气、每日委托与每周房租。玩家可在三处住处间搬家，租金与睡眠恢复不同；房租到期现金不足会结束本局。浏览器闲置不会让游戏时间自行流逝。 |
...
当前格式是 `schemaVersion: 4`。迁移逻辑支持能够识别的重建版 v1/v2/v3/v4 字段；本机 v3 存储会自动迁移到 v4，旧档默认住青藤小屋。**没有验证过原仓库旧存档格式**，不能保证旧存档可直接导入。不要为了通过校验，简单修改未知旧存档的版本号。
...
本次住房与房租修改的 PR 验证中，**100 项 Node 规则/存档/本机 HTTP 测试通过**，并成功完成静态构建。重建版此前另有 **11 项离线浏览器交互检查通过**；本次住房界面改动未重新执行该浏览器脚本。自动连续游玩仍覆盖 40 单配送、修为推进与守城结局可达性。
