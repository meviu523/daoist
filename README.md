...
| 模块 | 当前实现 |
| --- | --- |
| 地图配送 | 青岚城 72 个地点，9×8 正交路网，包含 9 个常驻服务/住宅地点与 63 个潜在配送目的地；常驻地点也可能收到订单。普通配送点仅在有任务时显示。 |
| 合并配送流程 | 点击地图任务点查看要求、距离、预计耗时、电量与体力，确认后直接配送，不另设取餐流程。 |
| 位置与时间 | 游戏连续运行，人物沿道路和桥梁逐步移动，途中可停止或改道；路程、耗时与电量/体力/耐久同步。 |
| 动态世界 | 每 15 游戏分钟及世界行动完成后补充订单空缺，已接单不被刷新删除；有昼夜、天气、每日委托与每周房租。玩家可在三处住处间搬家，租金与睡眠恢复不同；房租到期现金不足会结束本局。运行时即使不操作也会计时；切到后台不会自动切成“暂停”，但浏览器停止动画期间不补算离线时间，返回后按离开前状态继续。 |
...
| 修仙 | 六个大境界，每个细分一重至九重；突破先升小境界，九重后再进入下一大境界，化神九重封顶。包含吐纳、静修、淬体、炼药师、心法和法器；炼药需先在长乐集购鼎，熟练度逐步解锁丹方，高阶鼎提升成丹率与额外产出。 |
...
晚上进入“修行”收益更高。六个大境界都分为一重至九重；修为达到当前门槛后突破到下一重，九重突破成功才进入下一大境界一重。原地修炼无需赶路，前往听雨观则有额外灵地收益。拜访人物会先计算去其所在地的距离，再加交谈本身的时间。准备好后，在系统委托页领取阶段奖励，在“归途”查看结局条件。

炼药师路线从长乐集开始：先准备 ¥160 购买青铜药鼎，再用灵草与灵力开炉。回春丹开局即可尝试，之后通过开炉积累熟练度，逐步掌握清心散、凝气丹和破境丹。药鼎可以继续升级；赤铜温灵鼎和玄铁聚火鼎分别要求炼气、筑基境界，能提高成丹率，并增加一炉得到两份成品的机会。

主线不会在开局一次性倾倒设定：完成若干配送、进入更高境界、结识人物后，会依次触发“街脉显形 → 城市背面 → 灯火阵点 → 系统质疑 → 系统真相”。高阶系统兑换也随境界解锁，让成长和剧情同步展开。

地图支持单指/鼠标拖动、滚轮或双指缩放；`+`、`−` 缩放，定位按钮回到玩家，全图按钮查看整座城市。地图标记可用键盘聚焦后按 Enter，方向键可以平移地图。路线只在预览或移动过程中显示，到达目的地后立即从地图清除。
...
面板和剧情自动暂停，关闭后恢复打开前的状态；不会解除手动暂停。切后台或锁屏不会新增手动暂停；浏览器后台停止动画的时间不补算，返回后自动恢复离开前的运行/暂停状态。读档仍默认暂停。睡眠、充电、休息、修炼按进度恢复/增长；炼药和突破开始时扣投入，中断不退。炼药中断不涨熟练度，完成后才判定成丹；突破正常失败会退回约 75% 修为。耗时剧情选择提交后不可取消。
...
当前格式是 `schemaVersion: 7`。迁移逻辑支持能够识别的重建版 v1–v7 字段；新存储键不存在时自动迁移 v6/v5/v4/v3 本机存档，保留已有住处、车辆、人物关系、事件、炼药与途中行动。旧档没有小境界字段时从原大境界一重继续；若 v6 正处于旧规则的大境突破，则迁移为该大境界九重突破并保留旧投入，使目标仍是下一大境界。v5 若正处于炼药途中，会补一口基础药鼎保证该炉可继续；其他旧档默认未购鼎。进行中行动保留小数时间与道路坐标，不重复扣费或发奖。**没有验证过原仓库旧存档格式**，不能保证旧存档可直接导入。不要为了通过校验，简单修改未知旧存档的版本号。
