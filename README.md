...
这是根据项目对话记录重新实现的单机 Web RPG，版本 **1.2.0**，重建日期 **2026-09-18**。原账号读取返回 `403: Sorry. Your account was suspended`，没有取得旧仓库源码。因此，这是**玩法与需求重建**，不是原始代码、界面和剧情文本的逐字恢复。
...
| 修仙 | 六个境界，吐纳、静修、淬体、突破、炼药师、心法和法器；炼药需先在长乐集购鼎，熟练度逐步解锁丹方，高阶鼎提升成丹率与额外产出。成长线从凡人奔忙推进到城市暗面、护城阵、系统真相与最终选择。 |
...
当前格式是 `schemaVersion: 6`。迁移逻辑支持能够识别的重建版 v1–v6 字段；新存储键不存在时自动迁移 v5/v4/v3 本机存档，保留已有住处、车辆、人物关系、事件和途中行动。v5 若正处于炼药途中，会补一口基础药鼎保证该炉可继续；其他旧档默认未购鼎。进行中行动保留小数时间与道路坐标，不重复扣费或发奖。**没有验证过原仓库旧存档格式**，不能保证旧存档可直接导入。不要为了通过校验，简单修改未知旧存档的版本号。
